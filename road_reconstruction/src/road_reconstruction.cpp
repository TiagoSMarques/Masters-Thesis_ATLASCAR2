...
My_Filter::My_Filter()
{
  scan_sub_ = node_.subscribe<sensor_msgs::LaserScan>("/ld_rms/scan3", 100, &My_Filter::scanCallback, this);
...
void My_Filter::scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  if (!tfListener_.waitForTransform(
          scan->header.frame_id, "ground",
          scan->header.stamp + ros::Duration().fromSec(scan->ranges.size() * scan->time_increment), ros::Duration(1.0)))
  {
    return;
  }

  projector_.transformLaserScanToPointCloud("/ground", *scan, cloud, tfListener_);
